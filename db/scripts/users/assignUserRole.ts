...
import { userRoles } from "../../src/db/schema.js";
...
async function assignUserToRole(usernameOrEmail: string, role: string) {
  ...
  const roleEntity = await db.query.roles.findFirst({
    ...
    where: {
      AND: [{ name: role }, { deletedAt: { isNull: true } }],
    },
  });
...
async function main() {
  ...
  try {
    if (!values?.identifier || !values.role) {
      throw new Error(
        "Todos os argumentos são obrigatórios. Uso da função: bun --bun run scripts/users/assignUserRole.ts --identifier {{user_or_email}} --role {{role_name}}",
      );
    }
  ...
  console.log(
    `Usuário ${values.identifier} adicionado ao cargo ${values.role} com sucesso`,
  );
  return 0;
}
